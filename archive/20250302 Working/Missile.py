...
class Missile:
    def __init__(self, delay = 0, guidance = True):
        ...
        self.frontal_area = 0.03 #0.03  # m² (approximate cross-section)
        self.lift_area = 0.009 #0.09  # m² (reference area for lift)
    ...
    def control(self, measurement, timestep):
        ...
        if ((self.fuel<10) and (self.guidance == True)):
            # self.orientation = self.orientation - 0.2 * timestep*measurement.direction
            self.control_surface = self.control_surface + 2.0 * timestep * measurement.direction
